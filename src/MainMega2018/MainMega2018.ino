...
void loop() {//-------------------------------------------------------
  ...
  if (readVoltage) readTheVoltage();
  checkSwitches();
  checkTimers();
  if (relayChange) setRelays();
  if (displayChange) {
    updateDisplay();
    updateButtonLeds();
  }
...
void WagonStart() {//-------------------------------------------------------
  if ((millis() - wagonLastStart) > 20000) {
    ...
    Wagon.startRelay = true; relayChange = true;
  } else {
...
void checkTimers() {//-------------------------------------------------------
  ...
  if ((millis() - displayTimer) > 2000) {  //change display every 3 seconds
    displayMode ++;
    displayChange = true;
  }

  if (((millis() - lastEncoder) > 6000) & (menuItem[0] != 0) ) {  //return one menu every 6 seconds
    lastEncoder = millis();
    displayChange = true;
    if (menuItem[2] > 0) menuItem[2] = 0;
    else if (menuItem[1] > 0) menuItem[1] = 0;
    else menuItem[0] = 0;
  }

  if ((millis() - voltageTimer) > 30000) {  //every 30 Seconds
    voltageTimer = millis();
    readVoltage = true;
  }
  ...
  if ((millis() - (Flame.flameon + Flame.flameoff)) > lastFlame) flameReady = true;
    ...
    for (int i = 0; i < 5; i++) {
      if (fireFlame[i] && flameReady) {      //Turn on flame
        lastFlame = millis();
        flameReady = false;
        digitalWrite(relayBank2[i], LOW);
      }

      if ((millis() > (lastFlame + Flame.flameon)) && fireFlame[i]) { //turn off flame
    ...
    if (Wagon.stopRelay) {  //Wagon Engine kill reset after 5 seconds
      if ((millis() - wagonStopTime) > 5000) {
        Wagon.stopRelay = 0; relayChange = true;
      }
    }

    if (Wagon.startRelay) { //Wagon Engine Start reset after 5 seconds
      if ((millis() - wagonStartTime) > 5000) {
        Wagon.startRelay = 0; relayChange = true;
      }
    }
  }
...
void handleMainPress(String request) {//-------------------------------------------------------
  ...
  if (request == "btnMainLed1") Wagon.pwrLed1 = !Wagon.pwrLed1;
  if (request == "btnMainLed2") Wagon.pwrLed2 = (Wagon.pwrLed2) ? 0 : 1;
  if (request == "btnMainFlame") Wagon.pwrFlame = (Wagon.pwrFlame) ? 0 : 1;
  if (request == "btnMainSound") Wagon.pwrSound = (Wagon.pwrSound) ? 0 : 1;
  if (request == "btnMainHeadlight") Wagon.pwrHeadlight = (Wagon.pwrHeadlight) ? 0 : 1;
  if (request == "btnMainAux1") Wagon.pwrAux = (Wagon.pwrAux) ? 0 : 1;
...
void updateDisplay() {//-------------------------------------------------------
  ...
  if (displayColorShift) lcdColor(Background.huevalue, 2);
  ...
  if (menuItem[0] != 0) {
    ...
    if (WGps.locstring == "") WGps.locstring = "Earth";
...
void checkSwitches() {                 //-------------------------------------------------------
  ...
  if ((millis() - threeHundredMillis) > 300) {   //Run timer every 300 millis
    ...
    if ((x > oldSpeed + 2) || (x < oldSpeed - 2)) {
      ...
      txt = txt +  x;
      txt = txt + ",rngSpeed";
      serialEspFinal = txt; handleRequests();
    }
  }
  ...
  if ((millis() - lastRotaryPress) > 500) {   // wait x milliseconds between rotary presses
  ...
  for (int x = 0; x < 16; x++) {                                   //debounce first 11 pins
    if (digitalRead(digitalInputPin[x]) == HIGH) { // if pushed
      if (debouncePin(digitalInputPin[x])) {
        switch (x) {
          ...
          case 5:  //Flame Mode
            flameMode++;
            switch (flameMode) {
              case 1: flameModeTxt = "Front To Back"; break;
              case 2: flameModeTxt = "Back to Front"; break;
              case 3: flameModeTxt = "Knight Rider"; break;
              case 4: flameModeTxt = "All On"; break;
              default:
                flameModeTxt = "Random";
                flameMode = 0;
            }
        ...
        if (millis() < (lastDigitalButton + 500)) return;
        switch (x) {
          case 6: //Fire Mode
            switch (flameMode) {
              case 0: flameCount = 5; break;
              case 1: flameCount = 5; break;
              case 2: flameCount = 5; break;
              case 3: flameCount = 9; break;
              case 4: flameCount = 1; break;
            }
            break;
          case 7: //Color
            serialLed.print("L,btnBackground\r"); break;
          case 8: //Chase
            serialLed.print("L,btnChase\r"); break;
          case 9: //Sparkle
            serialLed.print("L,btnSparkle\r"); break;
          case 10: //Noise
            serialLed.print("L,btnNoise\r"); break;
          case 11:
            relayChange = true;
  ...
  if (x != encoderPos) {
    ...
    if (menuItem[0] != 0) {
      if (menuItem[1] == 0) {
        ...
        if (menuItem[0] > Main_Menu_Max_Root) menuItem[0] = Main_Menu_Max_Root;
        if (menuItem[0] < 1) menuItem[0] = 1;
      } else if (menuItem[2] == 0) {
        menuItem[1] += encoderPos;
        if (menuItem[1] > Main_Menu_Max_Second) menuItem[1] = Main_Menu_Max_Second;
        if (menuItem[1] < 1) menuItem[1] = 1;
    ...
    if (menuItem[2] == 1) {
      switch (menuItem[0]) {
        ...
        case 2:
          switch (menuItem[1]) {
            ...
            case 3: //hue
              ...
              txt += ",rngChaseHue\r";
              break;
            case 4: txt = "L,btnChaseColor\r"; break;
          }
          serialLed.print(txt);
          break;
        ...
        case 3:
          switch (menuItem[1]) {
            case 1: txt = "L,btnBackground\r"; break;
            case 2:
              txt = "L," ;
            ...
            case 3:
              ...
              break;
            case 4: txt = "L,btnBackgroundColor\r"; break;
          }
          serialLed.print(txt);
          break;
    ...
    } else { // Menu is off, Change preset display mode
      txt = "L,";
...
String menuItemPrint(int row) {//-------------------------------------------------------
  ...
  if (menuItem[1] == 0) {
    if ((menuItem[0] > 3) & (row != 0)) row = row + 3;
    switch (row) {
      case 0:
        txt = "--Main Menu--";
        return txt;
  ...
  if (menuItem[2] == 0) {
    if ((menuItem[1] > 3) & (row != 0))  row = row + 3;
    switch (menuItem[0])
    {
      case 1:// //Sparkle menu
        switch (row) {
          case 0:
            txt = "--Sparkle Menu--"; break;
          case 1:
            txt = "Sparkle: "; txt = Sparkle.on ? "On" : "Off"; break;
          case 2:
            txt = "Intensity: "; txt += Sparkle.value; break;
          case 3:
            txt = "Hue Value: "; txt += Sparkle.huevalue; break;
          case 4:
            txt = "Mode: "; txt += Sparkle.colorname; break;
        }
        break;
      case 2:
        switch (row) {
          case 0:
            txt = "--Chase Menu--"; break;
          case 1:
            txt = "Chase: "; txt += (Chase.on) ? "On" : "Off"; break;
          case 2:
            txt = "Distance: "; txt += Chase.distance; break;
          case 3:
            txt = "Hue Value: "; txt += Chase.huevalue; break;
          case 4:
            txt = "Mode: "; txt += Chase.colorname; break;
        }
        break;
      case 3:
        switch (row) {
          case 0:
            txt = "--Background Menu--"; return txt; break;
          case 1:
            txt = "Background: "; txt += Background.on ? "On" : "Off"; break;
          case 2:
            txt = "Brightness: "; txt += Background.value; break;
          case 3:
            txt = "Hue Value: "; txt += Background.huevalue; break;
          case 4:
            txt = "Mode: "; txt += Background.colorname; break;
        }
        break;
      case 4:
        switch (row) {
          case 0:
            txt = "--Wagon Settings--"; return txt; break;
          case 1:
            txt = "Set Gps Home"; break;
          case 2:
            txt = "Auto Start: "; txt += wagonAutoStart ? "Active" : "Disabled"; break;
          case 3:
            txt = "Auto Start Voltage"; break;
          case 4:
            txt = "Color Shift: "; txt += displayColorShift ? "On" : "Off"; break;
        }
        break;
    }
    if (row != 0) {
  ...
  if (menuItem[3] == 0) {
    ...
    {
      case 1:// //Sparkle menu--)
        if (row == 0) txt = "--Sparkle Menu--";
        if (row != 3) break;
        switch (menuItem[1]) //Sparkle Submenu
        {
          case 1: txt = (Sparkle.on) ? "Sparkle: On" :  "Sparkle: Off";        break;
          case 2: txt = "Value: " + (String)Sparkle.value; break;
          case 3: txt = "Color of Hue: " + (String)Sparkle.huevalue; break;
          case 4: txt = "Color Mode: " + Sparkle.colormode;   break;
        }
        break;

      case 2:
        if (row == 0) txt = "--Chase Menu--";
        if (row != 3) break;
        switch (menuItem[1]) // Submenu
        {
          case 1: txt = (Chase.on) ? "Chase: On" :  "Chase: Off";        break;
          case 2: txt = "Distance: " + (String)Chase.distance; break;
          case 3: txt = "Color of Hue: " + (String)Chase.huevalue; break;
          case 4: txt = "Mode: " + Chase.colormode;   break;
        }
        break;

      case 3:
        if (row == 0) txt = "--Background Menu--";
        if (row != 3) break;
        switch (menuItem[1]) // Submenu
        {
          case 1: txt = (Background.on) ? "Background: On" :  "Background: Off";        break;
          case 2: txt = "Brightness: " + (String)Background.value; break;
          case 3: txt = "Color of Hue: " + (String)Background.huevalue; break;
          case 4: txt = "Mode: "; txt += Background.colorname;  break;
        }
        break;

      case 4:
        if (row == 0) txt = "--Wagon Settings--";
        if (row != 3) break;
        switch (menuItem[1]) // Submenu
        {
          case 1: txt = "Set GPS Home: ";
            txt = txt + WGps.lat;
            txt = txt + " - ";
            txt = txt + WGps.lon;  break;
          case 2: txt = (wagonAutoStart) ? "Auto Start Engine: On" : "Auto Start Engine: Off"; break;
          case 3: txt = "Auto Start Voltage: " + (String)Wagon.startVoltage;  break;
          case 4: txt = (displayColorShift) ? "Color Shift: On" : "Shift: Off"; break;
        }
        break;
    }
...
void readTheVoltage() {                 //-------------------------------------------------------
  ...
  if (wagonAutoStart & (Wagon.mainVoltage < Wagon.startVoltage)) WagonStart();
}

void welcomeScreen() {//-------------------------------------------------------
  btnCircle(30);
  lcdCenter("AntiDysentery", 0, 2); updateLcd(2);
  for (int x = 0; x < 255; x++) {
    lcdColor(x, 2);
    delay(4);
  }
  btnFTB(50);
  lcdCenter("JankyTown", 1, 2); updateLcd(2);
  for (int x = 0; x < 255; x++) {
    lcdColor(x, 2);
    delay(3);
  }
  btnRandom(40);
  lcdCenter("Burning Man", 2, 2); updateLcd(2);
  for (int x = 0; x < 255; x++) {
    lcdColor(x, 2);
    delay(2);
  }
  btnCenterOut(20);
  lcdCenter("2018", 3, 2); updateLcd(2);
  for (int y = 0; y < 10; y++) {
    for (int x = 0; x < 255; x++) {
      lcdColor(x, 2);
      delay(1);
...
void lcdPrint(String txt, int row, int device) {//-------------------------------------------------------
  ...
  if ((device == 0) || (device == 2)) lcdFlame[row] = txt;
  if ((device == 1) || (device == 2)) lcdMain[row] = txt;
}
...
void updateLcd(int device) {//-------------------------------------------------------
  if ((device == 0) || (device == 2)) {
    if (lcdFlame[0].length() == 0) lcdFlame[0] = "                    ";
    if (lcdFlame[1].length() == 0) lcdFlame[1] = "                    ";
    if (lcdFlame[2].length() == 0) lcdFlame[2] = "                    ";
    if (lcdFlame[3].length() == 0) lcdFlame[3] = "                    ";
  ...
  if ((device == 1) || (device == 2)) {
    if (lcdMain[0].length() == 0) lcdMain[0] = "                    ";
    if (lcdMain[1].length() == 0) lcdMain[1] = "                    ";
    if (lcdMain[2].length() == 0) lcdMain[2] = "                    ";
    if (lcdMain[3].length() == 0) lcdMain[3] = "                    ";
...
void handleRequests() { //-------------------------------------------------------
  ...
  if (serialXbeeFinal != "") {
    lastXbee = millis();
    FlameOk = true;
    lcdCenter(serialXbeeFinal, 3, 0); updateLcd(0);
    Serial.print("Incoming Flame: "); Serial.println(serialXbeeFinal);
  ...
  if (serialEspFinal != "") {
    ...
    Serial.print("Incoming Esp: "); Serial.println(serialEspFinal);
...
void btnCenterOut(int del) {
  for (int x = 0 ; x < 9; x++) {
    switch (x) {
      case 0: sr.set(mapled[6], HIGH); break;
      case 1: sr.set(mapled[5], HIGH); sr.set(mapled[7], HIGH); break;
      case 2: sr.set(mapled[4], HIGH); sr.set(mapled[8], HIGH); break;
      case 3: sr.set(mapled[3], HIGH); sr.set(mapled[9], HIGH); break;
      case 4: sr.set(mapled[2], HIGH); sr.set(mapled[10], HIGH); break;
      case 5: sr.set(mapled[1], HIGH); sr.set(mapled[11], HIGH); break;
      case 6: sr.set(mapled[0], HIGH); sr.set(mapled[12], HIGH); sr.set(mapled[14], HIGH); break;
      case 7: sr.set(mapled[13], HIGH); sr.set(mapled[15], HIGH);  break;
    }
    delay(del);
  }
  for (int x = 0 ; x < 8; x++) {
    switch (x) {
      case 0: sr.set(mapled[6], LOW); break;
      case 1: sr.set(mapled[5], LOW); sr.set(mapled[7], LOW); break;
      case 2: sr.set(mapled[4], LOW); sr.set(mapled[8], LOW); break;
      case 3: sr.set(mapled[3], LOW); sr.set(mapled[9], LOW); break;
      case 4: sr.set(mapled[2], LOW); sr.set(mapled[10], LOW); break;
      case 5: sr.set(mapled[1], LOW); sr.set(mapled[11], LOW); break;
      case 6: sr.set(mapled[0], LOW); sr.set(mapled[12], LOW); sr.set(mapled[14], LOW); break;
      case 7:  sr.set(mapled[13], LOW); sr.set(mapled[15], LOW); break;
    }
    delay(del);
  }
}
